...
def fit_and_evaluate_datasets(
    ...
    """
    Similar to 'fit_and_evaluate', but uses dataset objects instead of directory paths.

    Args:
        model_cls (type): Model constructor/class.
        train_dataset (Dataset): Pre-constructed training dataset.
        val_dataset (Dataset): Pre-constructed validation (or test) dataset.
        device (torch.device): CPU or GPU device.
        num_channels (int): EEG channels.
        num_classes (int): Number of classes.
        seq_length (int): Sequence length (unused here if dataset is already formed).
        num_epochs (int): Number of epochs for training.
        batch_size (int): Batch size.
